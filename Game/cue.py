...
class Cue(pygame.sprite.Sprite):
    def __init__(self, target):
        ...
        self.sprite_size = np.repeat([config.cue_length + config.cue_max_displacement], 2)
        self.clear_canvas()
    ...
    def update(self, *args):
        if self.visible:
            ...
            initial_coords = np.array([math.sin(self.angle + 0.5 * math.pi),
                                       math.cos(self.angle + 0.5 * math.pi)]) * config.cue_thickness
    ...
    def update_cue_displacement(self, mouse_pos, initial_mouse_dist):
        displacement = physics.point_distance(mouse_pos, self.target_ball.ball.pos) - initial_mouse_dist + config.ball_radius
    ...
    def cue_is_active(self, game_state, events):
        ...
        initial_mouse_dist = physics.point_distance(initial_mouse_pos, self.target_ball.ball.pos)
    ...
    def update_cue(self, game_state, initial_mouse_dist, events):
        ...
        self.draw_lines(game_state, self.target_ball, prev_angle + math.pi, config.table_color)
        self.draw_lines(game_state, self.target_ball, self.angle + math.pi, (255, 255, 255))
        pygame.display.flip()

...
class GameState:
    ...
    def game_event_handler(self, event):
        if event.type == "POTTED":
            ...
            for ball in self.remaining_balls:
                if ball.number == event.data.number:
                    matching_ball = ball

            self.remaining_balls.remove(matching_ball)
    ...
    def reset_state(self):
        ...
        self.remaining_balls = pygame.sprite.Group()
    ...
    def generate_table(self):
        ...
        self.table_coloring = table_sprites.TableColoring(config.resolution, config.table_side_color, table_side_points)

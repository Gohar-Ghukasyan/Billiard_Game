...
def line_ball_collision_check(line, ball):
    if distance_less_equal(line.middle, ball.pos, line.length / 2 + config.ball_radius):
        ...
        return -config.ball_radius / 3 <= projected_distance <= np.hypot(*displacement_to_second_point) \
               + config.ball_radius / 3 and np.hypot(*(closest_line_point - ball.pos + line.line[0])) <= \

...
class BallSprite(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        if np.hypot(*self.ball.velocity) != 0:
            ...
            # angle formula is angle=((ball speed*2)/(pi*r*2))*2
            rotation_angle = -np.hypot(*self.ball.velocity) * 2 / (config.ball_radius * np.pi)
            transformation_matrix = physics.rotation_matrix(perpendicular_velocity, rotation_angle)
    ...
    def update_sprite(self):
        ...
        dist_from_centre = 1.1 - (self.label_offset[0] ** 2 + self.label_offset[1] ** 2) / (config.ball_radius ** 2)
        if self.label_offset[2] > 0:
            ...
            if self.label_offset[0] != 0:
                ...
                label = pygame.transform.scale(label, (int(config.ball_radius * dist_from_centre), config.ball_radius))
        ...
        for xy in itertools.product(range(config.ball_radius * 2 + 1), repeat=2):
            if is_outside[xy]:
                new_sprite.set_at(xy, (200, 200, 200))
